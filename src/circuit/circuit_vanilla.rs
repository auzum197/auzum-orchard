...
use halo2_gadgets::{
    ...
    sinsemilla::{
        HashDomains,
        chip::SinsemillaChip,
        merkle::{MerklePath, PreparedMerklePathWitness, prepare_merkle_path_witness},
    },
...
use crate::{
    circuit::{
        ANCHOR, AddressPoints, CMX, CV_NET_X, CV_NET_Y, Config, DISABLE_CROSS_ADDRESS,
        ENABLE_OUTPUT, ENABLE_SPEND, NF_OLD, OrchardCircuitVersion, RK_X, RK_Y,
    ...
    constants::{MERKLE_DEPTH_ORCHARD, OrchardFixedBasesFull, OrchardHashDomains},
    keys::{
        CommitIvkRandomness, DiversifiedTransmissionKey, NullifierDerivingKey, SpendValidatingKey,
    },
    note::{
        ExtractedNoteCommitment, Rho,
...
/// Where the Merkle path assignments of one synthesis get their prepared witness from.
pub(super) enum MerklePreparation {
    None,
    #[cfg(feature = "multicore")]
...
/// A [`CircuitVanilla`] whose Merkle path witness is prepared on another worker while
/// synthesis runs, so that its arithmetic overlaps the intervening assignments.
#[cfg(feature = "multicore")]
#[derive(Debug)]
pub(super) struct CircuitWithPreparedMerklePath {
    circuit: CircuitVanilla,
    prepared_merkle: Arc<Mutex<Option<mpsc::Receiver<Option<OrchardPreparedMerklePath>>>>>,
}

#[cfg(feature = "multicore")]
impl CircuitWithPreparedMerklePath {
    pub(super) fn new(circuit: &CircuitVanilla) -> Self {
        ...
        maybe_rayon::spawn(move || {
            let _ = sender.send(CircuitVanilla::prepare_merkle_path(
                preparation_path,
                preparation_pos,
                preparation_cm_old,
            ));
...
impl plonk::Circuit<pallas::Base> for CircuitWithPreparedMerklePath {
    ...
    fn without_witnesses(&self) -> Self {
        Self {
            circuit: CircuitVanilla::empty(self.circuit.circuit_version),
...
impl CircuitVanilla {
    ...
    fn synthesize_with_merkle_preparation(
        ...
        if self.circuit_version.is_zsa() {
...
mod tests {
    ...
    use crate::{
        ...
        circuit::{
            Circuit, CircuitVanilla, Instance, K, ORCHARD_K11_PARAMS, OrchardCircuitVersion, Proof,
            SingleVerifier, VerifyingKey, orchard_k11_params,
        },
    ...
    pub(in crate::circuit) fn generate_circuit_instance<R: Rng>(
        rng: R,
    ...
    fn halo2_instance_matches_prepared_commitment_shape() {
        ...
        assert_eq!(super::super::INSTANCE_COLUMNS, 1);
        assert_eq!(super::super::INSTANCE_ROWS, 10);
        assert_eq!(super::super::ENABLE_SPEND, 7);
        assert_eq!(super::super::ENABLE_OUTPUT, 8);
        assert_eq!(super::super::DISABLE_CROSS_ADDRESS, 9);
        assert_eq!(halo2_instance.len(), super::super::INSTANCE_COLUMNS);
        assert_eq!(halo2_instance[0].len(), super::super::INSTANCE_ROWS);
    ...
    fn zsa_statement_requires_supporting_key() {
        ...
        assert!(matches!(
            Proof::new(Vec::new()).verify(keys.verifying_key(), core::slice::from_ref(&instance)),
            Err(super::plonk::Error::InvalidInstances),
        ));
    }
