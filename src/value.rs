...
mod tests {
    ...
    use super::{
        MAX_NOTE_VALUE, ValueCommitTrapdoor, ValueCommitment, ValueSum,
    ...
    fn check_binding_signature(
        ...
        let bvk = (values
            .into_iter()
            .map(|(value, rcv, asset)| ValueCommitment::derive_with_asset(value, rcv, asset))
            .sum::<ValueCommitment>()
            - ValueCommitment::derive(zatoshi_value_balance, ValueCommitTrapdoor::zero())
            - arb_values_to_burn
                .iter()
                .map(|(value, _, asset)| {
                    ValueCommitment::derive_with_asset(*value, ValueCommitTrapdoor::zero(), *asset)
                })
                .sum::<ValueCommitment>())
        .into_bvk();
